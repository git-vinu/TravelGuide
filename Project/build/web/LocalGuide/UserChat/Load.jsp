...
<%
    String selQry = "select * from tbl_guide_user_chat tsc inner join tbl_user t on (tsc.to_user_id=t.user_id) or (tsc.from_user_id=t.user_id) where t.user_id='" + request.getParameter("id") + "' order by chat_id";
    //System.out.println(selQry);
    ResultSet rs = con.selectCommand(selQry);
    while (rs.next()) {

        if (rs.getString("to_guide_id").equals(session.getAttribute("lid"))) {
...
} else if (rs.getString("from_guide_id").equals(session.getAttribute("lid"))) {
...
<div class="chat-message is-sent">
    <img src="../../Assets/Files/<%=z.getString("guide_photo")%>" alt="">
    <div class="message-block">
        <span><%=z.getString("guide_name")%></span>
        <div class="message-text"><%=rs.getString("chat_content")%></div>
    </div>
</div>
<div class="chat-message" style="margin: 0px; padding: 0px" >
