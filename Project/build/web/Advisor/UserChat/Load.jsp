...
<%
    String selQry = "select * from tbl_advicor_user_chat tsc inner join tbl_user t on (tsc.to_user_id=t.user_id) or (tsc.from_user_id=t.user_id) where t.user_id='" + request.getParameter("id") + "' order by chat_id";
    //System.out.println(selQry);
    ResultSet rs = con.selectCommand(selQry);
    while (rs.next()) {

        if (rs.getString("to_advicor_id").equals(session.getAttribute("adid"))) {
...
} else if (rs.getString("from_advicor_id").equals(session.getAttribute("adid"))) {
